...
export class LyricsController {
  ...
  @Get('user/:userId')
  @ApiOperation({ summary: 'Get all lyrics for a user' })
  @ApiParam({ name: 'userId', description: 'User ID' })
  @SwaggerResponse({
    status: HttpStatus.OK,
    description: 'User lyrics have been successfully retrieved.',
  })
  async findByUser(
    @Param('userId') userId: string,
  ): Promise<ApiResponse<Lyrics[]>> {
    return this.lyricsService.findByUser(userId);
  }
}
