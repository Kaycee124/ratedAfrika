...
import { FileBase, FileStatus } from '../entities/file-base.entity';
import {
  AudioFile,
  AudioFormat,
  AudioQuality,
...
export class StorageService {
  ...
  async initiateMultipartUpload(
    ...
    try {
      ...
      // Generate a temporary key and path for the file
      const tempKey = `temp_${uploadId}/${filename}`;
      const tempPath =
        this.provider instanceof LocalStorageProvider
          ? join(this.provider.getBasePath(), tempKey)
          : `https://${process.env.STORAGE_BUCKET || 'your-bucket'}.s3.${
              process.env.AWS_REGION
            }.amazonaws.com/${tempKey}`;

      const format = this.getMimeTypeFormat(
        options.metadata?.mimeType || filename,
      );
      ...
      switch (options.type) {
        case 'audio':
          fileEntity = this.audioFileRepository.create({
            ...
            sampleRate: 0,
            channels: 0,
            waveform: [], // Initialize empty waveform array
            analysis: {
              peakAmplitude: 0,
              dynamicRange: 0,
              clipCount: 0,
        ...
        case 'video':
          fileEntity = this.videoFileRepository.create({
            ...
            frameRate: 0,
            bitrate: 0,
            // Ensure videoStream is a properly formatted JSON object
            videoStream: {
              codec: 'pending',
              bitrate: 0,
              frameRate: 0,
